<script>
    import { total_time, p2_ellapsed_time, p2_last_time, p2_isRunning, p1_isRunning } from './stores.js';
	...
	const unsubscribeStartTime = p2_last_time.subscribe(value => {
		lastTime = value;
	});

	const unsubscribeIsRunning = p2_isRunning.subscribe(value => {
		isRunning = value;

		if (isRunning) {
			interval = setInterval(() => {
				if (lastTime === 0) p2_last_time.set(Date.now());
				p2_ellapsed_time.update(x => x + (Date.now() - lastTime));
				p2_last_time.set(Date.now());
			}, 100);
		}
		else {
			p2_last_time.set(0);
	...
	const unsubscribeOtherIsRunning = p1_isRunning.subscribe(value => {
	...
	// if isRunning, pause it and start the other player
